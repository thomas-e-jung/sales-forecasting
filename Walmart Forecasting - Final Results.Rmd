...
# Normalize data
min_value <- min(df_reg$Weekly_Sales)
max_value <- max(df_reg$Weekly_Sales)
#min_max <- matrix(c(min(df_reg$Weekly_Sales), max(df_reg$Weekly_Sales)), nrow = 421570, ncol = 2, byrow = TRUE)
df_reg <- df_reg %>%
  mutate(Weekly_Sales_Norm = (Weekly_Sales-min_value)/(max_value-min_value)) %>%
  dplyr::select(-Weekly_Sales)

# Check for normality
ad.test(df_reg$Weekly_Sales_Norm)
# p-value <2e-16; non-normal distribution

# Check for non-constant variance (heteroskedasticity) & non-normality
model_1 <- lm(Weekly_Sales_Norm ~ ., data = df_reg)
summary(model_1)
# R2 = 0.658,
# p-value < 2e-16
par(mfrow=c(2,2))
...
X <- df_reg %>%
  dplyr::select(-Weekly_Sales_Norm)
Y <- df_reg %>%
  dplyr::select(Weekly_Sales_Norm)
...
model_1 <- lm(Weekly_Sales_Norm ~ .-Type, data = df_reg)
attributes(alias(model_1)$Complete)$dimnames[[1]]
model_1 <- lm(Weekly_Sales_Norm ~ .-Type -Size, data = df_reg)
vif(model_1)
model_1 <- lm(Weekly_Sales_Norm ~ .-Type -Size -CPI, data = df_reg)
vif(model_1)

model_2 <- lm(Weekly_Sales_Norm ~ .-Type -Size -CPI, data = df_reg)
summary(model_2)
# R2 = 0.658,
# p-value < 2e-16
...
# Train & Test sets (train on first 24 months, reserve last 8 months for testing)
...
# Errors
errors <- prediction - test_set$Weekly_Sales_Norm
hist(errors)

# Root Mean Squared Error
pred_rmse <- RMSE(prediction, test_set$Weekly_Sales_Norm)
paste("RMSE:", pred_rmse)

# Percentage of cases with less than 30% error
rel_change <- 1 - ((test_set$Weekly_Sales_Norm - abs(errors)) / test_set$Weekly_Sales_Norm)
pred30 <- table(rel_change<0.30)["TRUE"] / nrow(test_set)
...
# weekNumMatrix <- model.matrix(~week_number - 1, df)
...
# Train & Test sets (random sampling from all data)
...
mse <- mean((test_values - pred_values)^2)
mae = caret::MAE(test_values, pred_values)
...
mse <- mean((actual_values - pred_values)^2)
mae = caret::MAE(actual_values, pred_values)
